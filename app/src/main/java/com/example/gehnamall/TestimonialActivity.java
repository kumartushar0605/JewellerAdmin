...
public class TestimonialActivity extends AppCompatActivity {
    ...
    private void fetchExistingBanners() {
        ...
        Request request = new Request.Builder()
                .url("http://3.110.34.172:8080/api/testimonial")
                .get()
                .build();
        ...
        client.newCall(request).enqueue(new Callback() {
            ...
            public void onFailure(@NonNull Call call, @NonNull IOException e) {
                runOnUiThread(() -> Toast.makeText(TestimonialActivity.this, "Failed to fetch banners", Toast.LENGTH_SHORT).show());
            }
                ...
                if (response.isSuccessful()) {
                    try {
                        ...
                        for (int i = 0; i < bannersArray.length(); i++) {
                            ...
                            int bannerId = bannerObject.getInt("testimonialId");
                    ...
                    runOnUiThread(() -> Toast.makeText(TestimonialActivity.this, "Error: " + response.code(), Toast.LENGTH_SHORT).show());
                }
            }
        });
    }
    ...
    private void removeBanner(BannerItem bannerItem) {
        if (bannerItem.isFromBackend()) {
            OkHttpClient client = new OkHttpClient();
            Request request = new Request.Builder()
                    .url("http://3.110.34.172:8080/admin/delete/testimonial/" + bannerItem.getBannerId())
                    .delete()
            ...
            client.newCall(request).enqueue(new Callback() {
                @Override
                public void onFailure(@NonNull Call call, @NonNull IOException e) {
                    runOnUiThread(() -> Toast.makeText(TestimonialActivity.this, "Failed to delete banner", Toast.LENGTH_SHORT).show());
                    Log.e("Delete", "Error deleting banner", e);
                }
                ...
                public void onResponse(@NonNull Call call, @NonNull Response response) {
                    if (response.isSuccessful()) {
                        ...
                        Log.e("Delete", "Failed response: " + response.body().toString());
                    }
                }
            });
        } else {
    ...
    private void submitData() {
        ...
        Request request = new Request.Builder()
                .url("http://3.110.34.172:8080/admin/upload/testimonial")
                .post(requestBody)
                .addHeader("Authorization", "Bearer " + token)
                .build();
        ...
        client.newCall(request).enqueue(new Callback() {
            @Override
            public void onFailure(@NonNull Call call, @NonNull IOException e) {
                runOnUiThread(() -> Toast.makeText(TestimonialActivity.this, "Failed to upload banners", Toast.LENGTH_SHORT).show());
            ...
            public void onResponse(@NonNull Call call, @NonNull Response response) throws IOException {
                if (response.isSuccessful()) {
                    runOnUiThread(() -> {
                        Toast.makeText(TestimonialActivity.this, "Testimonial uploaded successfully!", Toast.LENGTH_SHORT).show();
                        fetchExistingBanners(); // Refresh the banner list
                    });
                } else {
                    runOnUiThread(() -> Toast.makeText(TestimonialActivity.this, "Failed to upload banners", Toast.LENGTH_SHORT).show());
                    Log.e("Testimonials", "Failed response: " + response.body().string());
                }
            }
        });
    }
